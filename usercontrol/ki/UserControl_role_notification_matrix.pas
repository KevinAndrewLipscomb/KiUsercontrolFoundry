...
begin
  ...
  if session['UserControl_role_notification_matrix.p'] <> nil then begin
    ...
    p.be_loaded := IsPostBack and (string(session['UserControl_member_binder_UserControl_config_PlaceHolder_content']) = 'UserControl_role_notification_matrix');
    //
  end else begin
    //
...
procedure TWebUserControl_role_notification_matrix.DropDownList_filter_SelectedIndexChanged(sender: System.Object;
  e: System.EventArgs);
begin
  p.filter := Safe(DropDownList_filter.selectedvalue,NUM);
  Bind;
...
var
  ...
  i: cardinal;
begin
  p.biz_role_notification_map.Bind(p.filter,p.sort_order,p.be_sort_order_ascending,GridView_control,p.crosstab_metadata_rec_arraylist);
